...
int main(int argc, char** argv) {
  ...
  // arg 3: pairs file
  ifstream countfile(argv[3]);

  // arg 4: minimum average value across row
  stringstream minrowavg_arg(argv[4]);
  double minrowavg;
...
//  ofstream outmatrf(argv[6]);
  ...
  while (getline(countfile,line)) {
    ...
    if ((cumbins.count(chr1) == 1) && (cumbins.count(chr2) == 1)) {
      bin1 = cumbins[chr1] + pos1/bsize;
      bin2 = cumbins[chr2] + pos2/bsize;
      if (bin1 != bin2) {
